...
function display_date_timezone($time)
{
    ...
    try {
        return \Cake\I18n\Time::instance($time)->i18nFormat('yyyy-MM-dd HH:mm:ss', get_option('timezone', 'UTC'), null);
...
function request_count($typeReport){
    $conn = \Cake\Datasource\ConnectionManager::get('default');
    $buyerReport = $conn->newQuery()->select('*')->from('buyer_reports')->where(['status' => 0])->execute()->rowCount();
    $memberReport = $conn->newQuery()->select('*')->from('member_reports')->where(['status' => 0])->execute()->rowCount();
    if ($typeReport == 'buyer') return $buyerReport;
    if ($typeReport == 'member') return $memberReport;
    if ($typeReport == 'total') return $buyerReport + $memberReport;
}
...
function group_admin(){
    return ['admin','admin_report','admin_camp'];
}
function access_dashboard(){
    return ['member','admin','admin_report','admin_camp'];
}
function is_admin(){
    return ['admin'];
}
function is_admin_report(){
    return ['admin','admin_report'];
}
function is_admin_camp(){
    return ['admin','admin_camp'];
}