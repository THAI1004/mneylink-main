...
class AdvancedController extends AppAdminController
{
...

    {
        ...
        $this->loadModel('Statistics');
        $now = Time::now();
        $startOfMonth = $now->startOfMonth()->i18nFormat('yyyy-MM-dd HH:mm:ss');
        $this->Statistics->deleteAll(['created <' => $startOfMonth]);



            $this->Flash->success(__('Dữ liệu trước ngày '.$startOfMonth.' đã được xóa thành công!'));
