...
//            $withdraw = json_decode($withdraw);
            $withdraw_data = [
                ...
                'publisher_earnings' => $withdraw->publisher_earnings,
                'referral_earnings' => $withdraw->referral_earnings,
                'amount' => $withdraw->amount,
                'json_data' => $withdraw->json_data,
                'method' => $withdraw->method,
                'account' => $withdraw->account,
                'user_note' => $withdraw->user_note,
                'transaction_id' => $withdraw->transaction_id,
                'created' => $withdraw->created
            ];
//            $this->Withdraws->save($withdraw);
//            $this->update($this->table,['id' => $id],$withdraw);
            $s_withdraw = $this->Withdraws->patchEntity($withdraw,$withdraw_data);
            $this->Withdraws->save($s_withdraw);
        }
    }
    ...
    */
    ...
    public function cancel($id)
    {
        ...
        $withdraw_updated = $this->update($this->table,['id' => $id],json_decode($withdraw,true));
    ...
    public function bulkEdit(){
        ...
        if (empty($ids)) return $this->redirect(['action' => 'index']);
        $ids = explode(',',$ids);
        $withdraws = $this->Withdraws->find()->where(['id IN' => $ids]);
        $withdraws = $withdraws->toArray();
        if (empty($withdraws)) return $this->redirect(['action' => 'index']);
        $newIds = [];

        foreach ($withdraws as $withdraw){
            if ($action == 'approve' && $withdraw->status == 2){
                $withdraw->status = 1;
            }
            if ($action == 'complete' && $withdraw->status == 1){
                $withdraw->status = 3;
            }
            $this->Withdraws->save($withdraw);
        }
