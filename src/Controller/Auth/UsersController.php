...
class UsersController extends AppController
{
    ...
    public function signin()
    {
        ...
        if ($this->getRequest()->is('post') || $this->getRequest()->getQuery('provider')) {
            ...
            if ($user) {
                // Make sure to fetch the full user data including password hash
                ...
                // Debug: Log session data to verify password is included
                \Cake\Log\Log::write('debug', 'User logged in: ' . $user['username']);
                \Cake\Log\Log::write('debug', 'User role: ' . $user['role']);
                \Cake\Log\Log::write('debug', 'Password in user array: ' . (isset($user['password']) ? 'YES' : 'NO'));
                ...
                if (in_array($user['role'], ['admin', 'admin_report', 'admin_camp'])) {

                    return $this->redirect(['controller' => 'Users', 'action' => 'dashboard', 'prefix' => 'admin']);
                } elseif ($user['role'] == 'buyer') {
                    return $this->redirect(['controller' => 'Users', 'action' => 'dashboard', 'prefix' => 'buyer']);
                } else {
    ...
    public function signup()
    {
        ...
        if ($this->getRequest()->is('post')) {
            ...
            $user->plan_id = 1;
            $user->role = $this->getRequest()->getData('role');
            $user->status = 2;
            // Auto active buer role
            if ($user->role == 'buyer') {
                $user->status = 1;
            }
            ...
            if ($this->Users->save($user)) {
                ...
                $this->Flash->success(
                    __('Tài khoản của bạn đã được tạo nhưng chưa thể đăng nhập, để kích hoạt vui lòng liên hệ admin qua tele <a href="https://t.me/admneylink"><b>T.me/admneylink</b></a>'),
                    ['escape' => false]
                );

                if ($user->role == 'buyer') return $this->redirect(['controller' => 'Users', 'action' => 'dashboard', 'prefix' => 'buyer']);
