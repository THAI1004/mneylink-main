...
class PhpExcelComponent extends Component {
    ...
    public function createExcel() {
        ...
        $this->objWorksheet->setTitle((!empty($this->data['title'])) ? $this->data['title'] : $this->defaults['sheet1Name']);
    ...
    function setSize($cell,$width,$auto = false){
        if ($auto){
            return $this->objWorksheet->getColumnDimension($cell)->setAutoSize($auto);
        } else {
            return $this->objWorksheet->getColumnDimension($cell)->setWidth($width);
        }

    }

    function mergerCell($cell){
        return $this->objWorksheet->mergeCells($cell);
    }
